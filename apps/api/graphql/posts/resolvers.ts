...
const queries = {
  hello: () => "hi",
  // await redis.set("mykey", "From Redis Lib");
  ...
  getAllApplied: async (parent, args: { id: Id }, ctx: GraphqlContext, info) => {
    const { id } = args;
    return await PostService.getAllApplied(id);
  },
...
const extraResolvers = {
  ...
  AllApplied: {
    Posts: async (args) => await PostService.getPostBySlug(args.postId),
  },
};
