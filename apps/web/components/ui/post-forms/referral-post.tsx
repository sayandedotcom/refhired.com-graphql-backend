...
import { CREATE_REFERRAL_POST } from "@/graphql/posts/mutations/create-referral-post";
...
export default function ReferralPost() {
  ...
  const [createPost, { data, error }] = useMutation(CREATE_REFERRAL_POST);
  ...
  async function onSubmit(values: z.infer<typeof referralPostValidator>) {
    ...
    try {
      createPost({
        variables: {
          payload: {
            ...
            accept: values.accept as never,
            acceptLimit: values.acceptLimit,
            companyName: values.companyName,
            description: values.description,
            expiresAt: values.expiresAt as never,
      ...
      console.log("post=====================", data?.createReferralPost);
      if (data?.createReferralPost.success) {
        toast(data?.createReferralPost.message, {
          description: "You have Sucessfully posted a Referral",
        });
        form.reset();
      } else {
        sonerToast({
          severity: "error",
          title: "Error Occured !",
          message: data?.createReferralPost.message,
        });
      }
    } catch (err) {
      console.log("err======================================", err);
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="$50-80k/year & 0.85-1% Equity" {...field} />
                  </FormControl>
                  <FormMessage />
                  <FormDescription>Select the compensation.</FormDescription>
                </FormItem>
              )}
            />
          </div>
